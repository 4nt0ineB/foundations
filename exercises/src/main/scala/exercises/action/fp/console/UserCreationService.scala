...
class UserCreationService(console: Console, clock: Clock) {
  ...
  val readName: IO[String] = writeLine("What's your name?").andThen(readLine)
  ...
  val readDateOfBirth: IO[LocalDate] = {
    ...
    for {
      ...
      line <- readLine
      date <- parseDateOfBirth(line).onError(_ => printError)
    } yield date
  }
  ...
  val readSubscribeToMailingList: IO[Boolean] = {
    ...
    for {
      ...
      line <- readLine
      subscribed <- parseLineToBoolean(line).onError(_ => printError)
    } yield subscribed
  }
  ...
  val readUser: IO[User] =
    for {
      ...
      now <- clock.now
      user = User(name, dateOfBirth, subscribed, createdAt = now)
