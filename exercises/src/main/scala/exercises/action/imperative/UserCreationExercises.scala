...
//  }
  ...
  def parseYesOrNo(line: String): Boolean = {
    line match {
      case "Y" => true
      case "N" => false
  ...
  def readUser(console: Console, clock: Clock): User = {
    ...
    var birth = readDateOfBirthRetry(console, maxAttempt = 3)
    var subscribed = readSubscribeToMailingListRetry(console, maxAttempt = 3)
    new User(name, birth, clock.now(), subscribed)
  }
  ...
  def readDateOfBirthRetry(console: Console, maxAttempt: Int): LocalDate = {
    retry(maxAttempt) {
      console.writeLine("What's your date of birth? [dd-mm-yyyy]")
      var line = console.readLine()
      onError(LocalDate.parse(line, dateOfBirthFormatter),
