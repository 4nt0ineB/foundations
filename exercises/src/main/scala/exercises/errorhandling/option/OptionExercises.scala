...
object OptionExercises {
  ...
  //    users.foldLeft(Set.empty[AccountId]) { (acc, user) =>
//      user.role.getAccountId match {
//        case Some(accountId) => acc + accountId
//        case None => acc
//      }
//    }.toList
  ...
  def sequence[A](options: List[Option[A]]): Option[List[A]] = {
    options.foldLeft(Option(List.empty[A])) { (acc, opt) =>
      opt.zip(acc).map { case (value, tail) => value :: tail}
    }.map(_.reverse)
  }
