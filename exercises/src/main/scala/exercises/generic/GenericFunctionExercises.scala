...
object GenericFunctionExercises {
  ...
  case class Pair[A](first: A, second: A) {
    ...
    def zipWith[Other, To](other: Pair[Other])(combine: (A, Other) => To):
      Pair[To] = Pair(combine(first, other.first), combine(second, other.second))
  }
  ...
  def map3[A, B, C, D, To](p: Pair[A])(p1: Pair[B])(f1: (A, B) => C)(p2: Pair[D])(f2: (C, D) => To): Pair[To] =
    p.zipWith(p1)(f1).zipWith(p2)(f2)
  ...
  case class Predicate[A](eval: A => Boolean) {
    ...
    def contramap[From](zoom: From => A): Predicate[From] = Predicate(u => eval(zoom(u)))
  ...
  lazy val isValidUser: Predicate[User] = {
    ...
    byUser(_.age)(isBiggerThan(18)) &&
      byUser(_.name)(isLongerThan(3)) && isUsernameCapitalised
  }

  def byUser[To](zoom: User => To)(predicate: Predicate[To]): Predicate[User] =
    Predicate(u => predicate(zoom(u)))

  private val isAdult: Predicate[User] = isBiggerThan(3).contramap(_.age)
  private val isUsernameLongerThan3: Predicate[User] = isBiggerThan(3).contramap(_.name.length)
  private val isUsernameCapitalised: Predicate[User] = Predicate(_.name.charAt(0).isUpper)
  ...
  val longDecoderSAM: JsonDecoder[Long] = (json: Json) => json.toLong
  ...
  // 3d. Move `map` inside `JsonDecoder` trait so that we can use the syntax
  ...
  def optionDecoder[A](decoder: JsonDecoder[A]): JsonDecoder[Option[A]] =
    (json: Json) => {
      Try(decoder.decode(json)).toOption
    }
