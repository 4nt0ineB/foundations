...
import exercises.generic.GenericFunctionExercises.Predicate
...
object ValueFunctionExercises {
  ...
  def isValidUsername(username: String): Boolean = username.forall(isValidUsernameCharacter)
  ...
  case class Point(x: Int, y: Int, z: Int) {
    ...
    def forAll(predicate: Int => Boolean): Boolean = predicate(x) && predicate(y) && predicate(z)
  }
}
