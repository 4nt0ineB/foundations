...
object TemperatureExercises {
  ...
  def averageTemperature(samples: ParList[Sample]): Option[Double] = {
    val (temp, count) = samples.parFoldMap(sample => (sample.temperatureFahrenheit, 1))(Monoid.sumDoubleInt)
    if(count == 0) None
    else Some(temp / count)
  }
  ...
  : To = {
    parList.partitions.map { part =>
      part.foldLeft(default)((acc, value) => combine(acc, value))
  ...
  def summaryParList(samples: ParList[Sample]): Summary =
    Summary(
      min = samples.reduceMap(identity)(Semigroup.maxBy(_.temperatureFahrenheit)), // samples.parFoldMap(Option(_))(Monoid.minSample),
      max = samples.reduceMap(identity)(Semigroup.minBy(_.temperatureFahrenheit)),
      sum = samples.parFoldMap(_.temperatureFahrenheit)(Monoid.sum[Double]),
      size = samples.parFoldMap(_ => 1)(Monoid.sum[Int])
    )
  ...
  private def sampleToSummary(sample: Sample): Summary =
    Summary(
      min = Some(sample),
      max = Some(sample),
